...
class GiphyListViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        favoritesIds = Set(favorites.map{$0.id})
    ...
    private func load() {
        do {
            let saved = try FileService.read(type: [Giphy].self, from: "favorites")
            favorites = saved
        } catch (let error) {
            print(error)
        }
    ...
    @objc private func save() {
        do {
            try FileService.write(self.favorites, to: "favorites")
        } catch (let error) {
            print(error)
        }
    }
    ...
    private func add(favorite: Giphy) {
        ...
        favoritesIds.insert(favorite.id)
    ...
    private func remove(favorite: Giphy) {
        ...
        favoritesIds.remove(favorite.id)
        ...
        if let index = searchCollectionView.indexPath(for: cell)?.row  {
            if isStar {
                ...
                guard let snapShot = cell.snapshotView(afterScreenUpdates: false),
                      let favoritesSuperView = favoritesHeader.superview else {
                    return
                }
